(function() {
  ...
  const v3 = {
    embedScript({ uuid, env, playerParams, type, domain }) {
      ...
      embedCode.src = `//${domain}.vidyard.${env}/${uuid}.js?v=3.1.1&type=${type}&${toQueryString(playerParams)}`;
      return embedCode;
    },
    ...
    lightbox(options) {
      document.head.appendChild(v3.embedScript({ ...options, type: 'lightbox' }));
      return v3.lightboxImage(options);
    },
  };

  const v4 = {
    embedScript({ domain, env }) {
      const script = document.createElement('script');
      script.type = 'text/javascript';
      script.src = `//${domain}.vidyard.${env}/embed/v4.js`;
      script.async = true;
      return script;
    },
    embedImage({ domain, env, uuid, playerParams, type }) {
      ...
      var params = { ...playerParams, width: 360, height: 260 };
